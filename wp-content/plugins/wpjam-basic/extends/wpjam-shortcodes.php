...
class WPJAM_Shortcode{
	...
	public static function youku($atts, $content=''){
		if(preg_match('#http://v.youku.com/v_show/id_(.*?).html#i',$content, $matches)){
			return '<iframe class="wpjam_video" '.self::video_hwstring($atts).' src="http://player.youku.com/embed/'.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
		}
	}

	public static function qqv($atts, $content=''){
		if(preg_match('#//v.qq.com/iframe/player.html\?vid=(.+)#i',$content, $matches)){
			return '<iframe class="wpjam_video" '.self::video_hwstring($atts).' src="http://v.qq.com/iframe/player.html?vid='.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
	...
	public static function bilibili($atts, $content='') {
		if(preg_match('#//www.bilibili.com/video/(.+)#i',$content, $matches)){
			return '<iframe class="wpjam_video" '.self::video_hwstring($atts).' src="//player.bilibili.com/player.html?bvid='.esc_attr($matches[1]).'" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>';
		}
	}

	public static function tudou($atts, $content=''){
		if(preg_match('#http://www.tudou.com/programs/view/(.*?)#i',$content, $matches)){
			return '<iframe class="wpjam_video" '.self::video_hwstring($atts).' src="http://www.tudou.com/programs/view/html5embed.action?code='. esc_attr($matches[1]) .'" frameborder=0 allowfullscreen></iframe>';
		}
	}
	...
	public static function sohutv($atts, $content=''){
		if(preg_match('#http://tv.sohu.com/upload/static/share/share_play.html\#(.+)#i',$content, $matches)){
			return '<iframe class="wpjam_video" '.self::video_hwstring($atts).' src="http://tv.sohu.com/upload/static/share/share_play.html#'.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
		}
	}
}
...
if(is_admin()){
	...
	add_action('wpjam_plugin_page_load', function($plugin_page){
		if($plugin_page != 'wpjam-shortcodes'){
			return;
		}

		wpjam_register_list_table('wpjam-shortcodes', [
			'title'		=> '短代码',
			'plural'	=> 'shortcodes',
			'singular'	=> 'shortcode',
