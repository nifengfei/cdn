...
class WPJAM_Field{
	...
	private static function field_callback($field, $args=[]){
		...
		if(isset($args['data_type']) && $args['data_type'] == 'option'){
			if($args['option_type'] == 'array'){
				$field['name']	= $args['option_name'].self::generate_sub_field_name($field['name']);
			}
		}
	...
	public  static function get_field_value($field, $args=[]){
		...
		if(in_array($type, ['view', 'br','hr']) && !is_null($default)){
			return $default;
		}
		...
		if($data_type == 'form'){
			...
			if($args['option_type'] == 'array'){
				$value	= wpjam_get_setting($args['option_name'], $name);
			}else{
	...
	public  static function fields_validate($fields, $values=[]){
		$data = [];
...
class WPJAM_Form extends WPJAM_Field{
	...
	public static function ajax_form($args){
		...
		if($fields = $args['fields']){
			...
			$args['echo']	= false;
			$output	.= self::fields_callback($fields, $args);
		}
