...
add_action('init',function(){
	...
	if(wpjam_basic_get_setting('disable_trackbacks')){
		$wp->remove_query_var('tb');
	}

	if(wpjam_basic_get_setting('disable_post_embed')){ 
		$wp->remove_query_var('embed');
	}
...
if(wpjam_basic_get_setting('google_fonts')){
	add_filter('wpjam_html', function($html){
		...
		if(wpjam_basic_get_setting('google_fonts') == 'custom'){
			foreach ($google_fonts_searchs as $google_fonts_key => $google_fonts_search) {
				if(wpjam_basic_get_setting($google_fonts_key)){
					$search[]	= $google_fonts_search;
					$replace[]	= str_replace(['http://','https://'], '//', $google_fonts_search);
				}
			}
		}elseif(wpjam_basic_get_setting('google_fonts') == 'ustc'){
			$search		= array_values($google_fonts_searchs);
			$replace	= [
	...
	if($excerpt_optimization != 2){
		add_filter('get_the_excerpt', function($text='', $post=null){
			if(empty($text)){
				...
				$text	= WPJAM_Post::get_excerpt($post, $length);
			}
