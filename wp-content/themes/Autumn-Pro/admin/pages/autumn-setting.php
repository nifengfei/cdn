...
jQuery(function($){
	...
	$('tr#tr_dark_logo').hide();
	
	$('body').on('change', '#div_head_dark_switch input', function(){
		$('tr#tr_dark_logo').hide();
		if ($(this).is(':checked')) {
			$('tr#tr_dark_logo').show();
		}			
	});

	if(document.getElementById("head_dark_switch").checked){
		$('tr#tr_dark_logo').show();
	}else{
		$('tr#tr_dark_logo').hide();
	}
...
add_filter('wpjam_theme_setting', function(){
	...
	$global_sub_fields		= [
		...
		'head_dark_switch'	=> '导航栏显示「夜间模式」，切换按钮',
	...
	$sections	= [
		'icon'	=>[
			...
			'fields'	=>[
				...
				'logo_set'		=> ['title'=>'网站图标', 'type'=>'fieldset',	'fields'=>[
					'logo'		=> ['title'=>'', 'type'=>'img',	'item_type'=>'url',	'size'=>'120*40', 'description'=>'网站 LOGO，尺寸：120x40'],
					'favicon'	=> ['title'=>'', 'type'=>'img',	'item_type'=>'url',	'description'=>'网站 Favicon'],
					'img_lazysizes'	=> ['title'=>'', 'type'=>'img',	'item_type'=>'url',	'description'=>'延迟加载-默认加载图像'],
				]],
		...
		'extend'	=>[
			...
			'fields'	=>[
				...
				'dark_logo'		=> ['title'=>'暗黑模式 - 网站Logo', 'type'=>'img', 'item_type'=>'url',	'description'=>'自定义暗黑模式下的网站Logo'],
