	...
	$(".fav").click(function() {
		...
		if (!_this.hasClass('unfav')) {
			...
			$.ajax({
				...
				success: function(data) {
					button.children('.count').html(data);
					_this.removeClass("unfav-no").addClass("unfav").attr("title", "已收藏，点击取消收藏");
					_this.children('i').attr('class', 'iconfont icon-collection_fill');
				}
			});
			return false;

		} else if (_this.hasClass('unfav')) {
			var button = $(this);
			$.ajax({
				url : xintheme.ajaxurl,
				data : {
				...
				success: function(data) {
					button.children('.count').html(data);
					_this.removeClass("unfav").addClass("unfav-no").attr("title", "点击收藏");
